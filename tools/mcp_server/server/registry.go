...
func ToolRegistry() []ToolDescriptor {
	return []ToolDescriptor{
		{Name: "lookup_symbol", Description: "Exact/normalized/fuzzy symbol lookup with explicit match status.", InputSchema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{"symbol_name": map[string]any{"type": "string", "minLength": 1}, "symbol_type": map[string]any{"type": "string", "enum": schema.SupportedSymbolTypes}, "include_examples": map[string]any{"type": "boolean"}, "include_related": map[string]any{"type": "boolean"}}, "required": []string{"symbol_name"}}, RequestFields: []string{"symbol_name", "symbol_type", "include_examples", "include_related"}, ResponseFields: []string{"exact_match", "symbol", "alternatives", "warnings"}},
		...
		{Name: "get_related_symbols", Description: "Traverse related symbols grouped by relation type.", InputSchema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{"symbol_name": map[string]any{"type": "string", "minLength": 1}, "relation_types": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string", "enum": schema.SupportedRelationTypes}}, "depth": map[string]any{"type": "integer", "minimum": 0, "maximum": 3}, "limit_per_relation": map[string]any{"type": "integer", "minimum": 1, "maximum": 50}}, "required": []string{"symbol_name"}}, RequestFields: []string{"symbol_name", "relation_types", "depth", "limit_per_relation"}, ResponseFields: []string{"source_symbol", "relations", "graph_summary", "warnings"}},
		...
		{Name: "find_usage_examples", Description: "Find symbol or pattern usage examples.", InputSchema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{"symbol_name": map[string]any{"type": "string", "minLength": 1}, "pattern_name": map[string]any{"type": "string", "minLength": 1}, "addon_filter": map[string]any{"type": "string", "minLength": 1}, "limit": map[string]any{"type": "integer", "minimum": 1, "maximum": 100}}, "anyOf": []map[string]any{{"required": []string{"symbol_name"}}, {"required": []string{"pattern_name"}}}}, RequestFields: []string{"symbol_name", "pattern_name", "addon_filter", "limit"}, ResponseFields: []string{"query_type", "total_matches", "examples"}},
		...
		{Name: "scaffold_addon_snippet", Description: "Generate snippets using documented APIs only.", InputSchema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{"task_description": map[string]any{"type": "string", "minLength": 1}, "desired_apis": map[string]any{"type": "array", "minItems": 1, "items": map[string]any{"type": "string", "minLength": 1}}, "output_style": map[string]any{"type": "string", "minLength": 1}, "include_xml": map[string]any{"type": "boolean"}, "include_comments": map[string]any{"type": "boolean"}}, "required": []string{"task_description"}}, RequestFields: []string{"task_description", "desired_apis", "output_style", "include_xml", "include_comments"}, ResponseFields: []string{"generated_artifacts", "used_documented_symbols", "avoided_symbols", "uncertainty_notes", "rationale"}},
	}
}
